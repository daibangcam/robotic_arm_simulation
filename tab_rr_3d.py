...
class TabRR3D(ttk.Frame):
    def __init__(self, parent, vcmd, left_width):
        super().__init__(parent)
        self.vcmd, self.LEFT_WIDTH = vcmd, left_width
        self.l1_var, self.l2_var = tk.StringVar(value="10.0"), tk.StringVar(value="20.0")
        ...
        self.t1_var, self.t2_var = tk.DoubleVar(value=45), tk.DoubleVar(value=30)
    ...
    def build_ui(self):
        ...
        ttk.Entry(lf1, textvariable=self.l1_var, width=10, validate="key", validatecommand=self.vcmd).grid(row=0, column=1, pady=2)
        ttk.Label(lf1, text="Link 2 (cm):").grid(row=1, column=0, sticky=tk.W, padx=(0, 10))
        ttk.Entry(lf1, textvariable=self.l2_var, width=10, validate="key", validatecommand=self.vcmd).grid(row=1, column=1, pady=2)
        ...
        for i, (min_v, max_v) in enumerate([(self.t1_min_var, self.t1_max_var), (self.t2_min_var, self.t2_max_var)]):
            ...
            ttk.Entry(lf_limit, textvariable=min_v, width=6, validate="key", validatecommand=self.vcmd).grid(row=i, column=1, pady=2, padx=(0, 5))
            ttk.Entry(lf_limit, textvariable=max_v, width=6, validate="key", validatecommand=self.vcmd).grid(row=i, column=2, pady=2)
        ...
        ttk.Entry(lf3, textvariable=self.x_var, width=12, validate="key", validatecommand=self.vcmd).grid(row=0, column=1, pady=2)
        ttk.Label(lf3, text="Tọa độ Y (cm):").grid(row=1, column=0, sticky=tk.W, padx=(0, 10))
        ttk.Entry(lf3, textvariable=self.y_var, width=12, validate="key", validatecommand=self.vcmd).grid(row=1, column=1, pady=2)
        ttk.Label(lf3, text="Tọa độ Z (cm):").grid(row=2, column=0, sticky=tk.W, padx=(0, 10))
        ttk.Entry(lf3, textvariable=self.z_var, width=12, validate="key", validatecommand=self.vcmd).grid(row=2, column=1, pady=2)
        ...
        self.lbl_link2 = tk.Label(lf4, text="▬ Cánh tay (Arm)", fg="red", font=("Segoe UI", 12, "bold"), anchor="w")
        self.lbl_link2.pack(fill=tk.X)
        tk.Label(lf4, text="★ Đầu công tác", fg="#DAA520", font=("Segoe UI", 12, "bold"), anchor="w").pack(fill=tk.X)
        tk.Label(lf4, text="○ Vùng làm việc (Vỏ cầu)", fg="darkorange", font=("Segoe UI", 12, "bold"), anchor="w").pack(fill=tk.X)

        lf5 = ttk.LabelFrame(control_frame, text="Mô tả cánh tay Robot", padding="5")
        lf5.pack(fill=tk.X, pady=3)
        desc_rr3d = "Cánh tay RR không gian (Spatial 2-DOF). Ứng dụng trong các hệ thống Pan-Tilt hoặc tay máy cơ bản."
        ttk.Label(lf5, text=desc_rr3d, style="Desc.TLabel", justify=tk.LEFT, wraplength=self.LEFT_WIDTH - 40).pack(fill=tk.X)
    ...
    def draw_robot(self, t1, t2):
        ...
        self.lbl_link2.config(text=f"▬ Cánh tay ({L2} cm)")
        p0, p1, p2 = H2Q_Lib.H2Q_RR_3D_Forward_Kinematics(L1, L2, t1, t2)

        u, v = np.linspace(0, 2 * np.pi, 30), np.linspace(0, np.pi, 30)
        X_sphere = L2 * np.outer(np.cos(u), np.sin(v))
        Y_sphere = L2 * np.outer(np.sin(u), np.sin(v))
        Z_sphere = L1 + L2 * np.outer(np.ones(np.size(u)), np.cos(v))
        ...
        self.ax.scatter([p0[0], p1[0]], [p0[1], p1[1]], [p0[2], p1[2]], color='green', s=100)
        self.ax.scatter([p2[0]], [p2[1]], [p2[2]], color='yellow', s=300, marker='*', edgecolors='goldenrod')

        limit = L1 + L2
        self.ax.set_xlim([-limit, limit]); self.ax.set_ylim([-limit, limit]); self.ax.set_zlim([0, limit])
        self.ax.set_xlabel('Trục X (cm)'); self.ax.set_ylabel('Trục Y (cm)'); self.ax.set_zlabel('Trục Z (cm)')
    ...
    def update_from_sliders(self):
        ...
        self.updating = True
        try:
            x, y, z = self.draw_robot(self.t1_var.get(), self.t2_var.get())
            self.x_var.set(f"{x:.3f}"); self.y_var.set(f"{y:.3f}"); self.z_var.set(f"{z:.3f}")
    ...
    def update_from_coords(self):
        ...
        try:
            ...
            res = H2Q_Lib.H2Q_RR_3D_Inverse_Kinematics(L1, L2, float(self.x_var.get()), float(self.y_var.get()), float(self.z_var.get()))
            if res[0] is not None:
                limits = [(float(self.t1_min_var.get()), float(self.t1_max_var.get())), (float(self.t2_min_var.get()), float(self.t2_max_var.get()))]
                if all(l[0] <= r <= l[1] for r, l in zip(res, limits)):
                    self.t1_var.set(round(res[0], 1)); self.t2_var.set(round(res[1], 1))
